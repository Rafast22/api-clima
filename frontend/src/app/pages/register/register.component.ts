...
export class RegisterComponent {
  ...
  async register() {
    ...
    if (this.formulario.valid) {
      await this.authService.register(user).then((r: any) => {
        this.notification.abrirNotificacionMensaje('Registrado con éxito', 'alert')
        this.authService.login(user).then((r) => this.route.navigateByUrl("/recomendaciones"))
      })
        .catch((err) => {
          this.notification.abrirNotificacionMensaje(err, 'alert')
        })
      return;
    }
  }
    ...
    switch (campo) {
      ...
      case 'password':
        if (this.password.hasError("required")) {
          ...
          this.errorMessagePassword.set('la contraseña debe contener como minimo 8 caracteres');
        }
        else {
